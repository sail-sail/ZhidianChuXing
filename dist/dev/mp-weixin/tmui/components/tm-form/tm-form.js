...
const _sfc_main = /* @__PURE__ */ common_vendor.defineComponent({
  ...
  setup(__props, { expose, emit: emits }) {
    const props = __props;
    const _modelVal = common_vendor.ref(common_vendor.index.$tm.u.deepClone(props.modelValue));
    const _backModelVal = common_vendor.index.$tm.u.deepClone(props.modelValue);
    common_vendor.watchEffect(() => {
      _modelVal.value = common_vendor.index.$tm.u.deepClone(props.modelValue);
    });
    ...
    const formFunCallBack = common_vendor.ref("validate");
    ...
    function getChangedField(oldValue, newValue) {
      const changedFields = [];
    ...
    function validate(fileds = [], type = "all") {
      ...
      for (let i = 0, len = par.length; i < len; i++) {
        ...
        if (type == "all") {
          ...
          for (let j = 0; j < vallist.length; j++) {
            if (!vallist[j].validator) {
              isPass = false;
              rulstVal.message = (_f = (_e = vallist[j]) == null ? void 0 : _e.message) != null ? _f : "校验通过";
              rulstVal.validator = (_h = (_g = vallist[j]) == null ? void 0 : _g.validator) != null ? _h : true;
              break;
            }
          }
          list.push(__spreadValues({ field: item.field }, rulstVal));
...
const Component = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__file", "D:/zhidianUniapp/src/tmui/components/tm-form/tm-form.vue"]]);
